...
constexpr auto root_digest = "9cb2879b1cc27e98d8def7c926a38e9f4eb2cbec90785be74c009156b4a1e4c5";
constexpr auto archive_digest = "1efa067767f3c729bbd67c40b3faa239673025f421133bddf32ec6b090231b09";
constexpr qint64 archive_byte_size = 5'244'039;
  ...
  public:
    ...
    [[nodiscard]] auto next(QStringView session_id, std::uint64_t next_sequence,
        ...
        return ui::RecordAccessTransitionStamp{
            QStringLiteral("ca4m4.opengrid.record-access.event.%1")
...
class OpenGridFoiaUiE2eTest final : public QObject {
    Q_OBJECT
...
void OpenGridFoiaUiE2eTest::installsFinalPackAndExercisesPublicAndAuthorizedRecord() {
    const model::PackRevision expected_root{model::PackId{"us.ca4.m4.opengrid-foia"}, "1.2.0",
                                            root_digest};
    ...
    const model::PackRevision expected_bench{model::PackId{"foundation.us-ca4-fictional-bench"},
                                             "1.0.0", bench_digest};
    const QDir root(QStringLiteral(APPELLATE_M4_OPENGRID_ROOT));
    const QDir foundations(QStringLiteral(APPELLATE_M4_FOUNDATIONS));

    QTemporaryDir state;
    QVERIFY(state.isValid());
    const auto root_archive = QDir(state.path()).filePath(QStringLiteral("opengrid-foia.awpack"));
    ...
    QCOMPARE(runtime_case.record.docket_entries.size(), std::size_t{84});
    QCOMPARE(runtime_case.record.page_anchors.size(), std::size_t{654});
    QCOMPARE(runtime_case.record.sealed_disclosures.size(), std::size_t{17});
    ...
    QCOMPARE(derived_public_partition.value(QStringLiteral("ca4m4.opengrid.docket.district")),
             qsizetype{28});
    QCOMPARE(derived_public_partition.value(QStringLiteral("ca4m4.opengrid.docket.appellate")),
             qsizetype{15});
    QCOMPARE(derived_public_partition.value(
                 QStringLiteral("ca4m4.opengrid.docket.counterfactual-branches")),
             qsizetype{24});
    const std::array public_docket_partitions{
        QStringLiteral("ca4m4.opengrid.docket.district"),
        QStringLiteral("ca4m4.opengrid.docket.appellate"),
        QStringLiteral("ca4m4.opengrid.docket.counterfactual-branches"),
    };
    ...
    const std::array public_tag_partitions{
        std::pair{QStringLiteral("lower_record"), qsizetype{28}},
        std::pair{QStringLiteral("actual_appellate_docket"), qsizetype{15}},
        std::pair{QStringLiteral("never_occurred_on_actual_docket"), qsizetype{24}},
    };
    for (const auto& [filter, expected_count] : public_tag_partitions) {
        workspace->setDocketFilter(filter);
        QCOMPARE(workspace->visibleDocketCount(), expected_count);
    }
    workspace->setDocketFilter(QStringLiteral("Sealed Highlighted Opening Brief"));
    ...
    const std::array public_navigations{
        Navigation{QStringLiteral("JA250"), QStringLiteral("ca4m4.opengrid.record.entry.l33"), 20,
                   0, QStringLiteral("operational playbooks were compiled")},
        Navigation{QStringLiteral("PA129"), QStringLiteral("ca4m4.opengrid.record.entry.a17"), 22,
                   0, QStringLiteral("affirm that limited ruling")},
        Navigation{QStringLiteral("PA340"), QStringLiteral("ca4m4.opengrid.record.entry.b26"), 20,
                   0, QStringLiteral("order no remand")},
    };
    ...
    constexpr auto disclosure_id = "ca4m4.opengrid.disclosure.actual-opening-brief";
    const auto stable_anchor =
        QStringLiteral("ca4m4.opengrid.anchor.stable.actual-opening-brief.page-01");
    const auto sealed_anchor = QStringLiteral("ca4m4.opengrid.anchor.pa39");
    const auto public_entry = QStringLiteral("ca4m4.opengrid.record.entry.a06");
    const auto sealed_entry = QStringLiteral("ca4m4.opengrid.record.entry.a07");
    ...
    QCOMPARE(window.recordAccessMenu()->actions().size(), 17);
    ...
    QVERIFY(!grant->isEnabled());
    QVERIFY(revoke->isEnabled());
    QCOMPARE(workspace->visibleDocketCount(), qsizetype{68});
    ...
    QCOMPARE(workspace->loadedPageCount(), 16);
    QTRY_COMPARE_WITH_TIMEOUT(workspace->currentPageIndex(), 0, 10'000);
    workspace->setDocumentSearch(QStringLiteral("semantic-placement counterpart"));
    QTRY_VERIFY_WITH_TIMEOUT(workspace->documentSearchResultCount() > 0, 10'000);
    ...
    QCOMPARE(workspace->loadedPageCount(), 16);
    QTRY_COMPARE_WITH_TIMEOUT(workspace->currentPageIndex(), 0, 10'000);
    workspace->setDocumentSearch(QStringLiteral("Nature of the Appeal"));
    QTRY_VERIFY_WITH_TIMEOUT(workspace->documentSearchResultCount() > 0, 10'000);
    QTRY_VERIFY_WITH_TIMEOUT(!pdf_search->resultsOnPage(0).isEmpty(), 10'000);
}
...
QTEST_MAIN(OpenGridFoiaUiE2eTest)

#include "tst_m4_opengrid_foia_ui_e2e.moc"
