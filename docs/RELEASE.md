...
also retained byte-for-byte as compatibility evidence. Cinder Lake v1.2.0 is an additional
non-default level-2 payload with qualified independent review pending.
...
runtime/plugins discovered by Qt's deployment API, six exact immutable pack archives,
...
archive gates require the exact six-path `.awpack` allowlist, so neither a generated starter nor
...
six manifest entries against their exact IDs, versions, archive hashes, root revisions, sizes,
...
In a separate catalog, the same gate installs those three foundations followed by Cinder Lake
v1.2.0. It requires the exact four-revision resolved closure and an offscreen desktop load, then
repeats both checks using only the relocated CLI, archive, and desktop. This proves exact archive
integrity, dependency resolution, runtime projection, and desktop loading for Cinder Lake. It does
not establish qualified independent review, level 3 or gold status, replace Asterglen as the
primary/default payload, or complete the nine-family MVP.
